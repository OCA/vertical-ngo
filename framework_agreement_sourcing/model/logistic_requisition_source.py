...
class logistic_requisition_source(orm.Model, FrameworkAgreementObservable):
    ...
    _columns = {
        ...
        'framework_agreement_po_id': fields.many2one(
            'purchase.order',
            'Agreement Purchase'
        ),
                ...
                if po_l_ids:
                    if len(po_l_ids) > 1:
                        raise orm.except_orm(
                            _('Many Purchase order lines found for %s') % line.name,
    ...
    def _prepare_purchase_order_line(self, cr, uid, po_id, line,
        ...
        if line.framework_agreement_id:
            price = line.framework_agreement_id.get_price(line.proposed_qty,
                                                          currency=currency)
        ...
        taxes = acc_pos_obj.map_tax(
            cr, uid,
            supplier.property_account_position,
            taxes_ids
        )
    ...
    def _make_po_from_source_lines(self, cr, uid, main_source, other_sources,
        ...
        for source in chain([main_source], other_sources):
            line_vals = self._prepare_purchase_order_line(
                cr, uid,
                po_id,
                source,
                supplier,
    ...
    def make_purchase_order(self, cr, uid, ids, pricelist, context=None):
        ...
        if len(agreement_sources) > 1:
            raise orm.except_orm(
                _('There should be only one agreement line'),
                _('Please correct selection')
            )
        if not main_source:
            raise orm.except_orm(
        ...
        if method != AGR_PROC:
            if proposed_product_id:
                ...
                if proposed_product_id:
                    ...
                    prod = prod_obj.browse(cr, uid, proposed_product_id,
                                           context=context)
                    value = {
    ...
    def onchange_agreement(self, cr, uid, ids, agreement_id, req_line_id, qty,
        ...
        currency = line_source.currency_id
        date = self._get_date(cr, uid, req_line_id, context=context)
        return self.onchange_agreement_obs(
            cr, uid, ids,
            agreement_id,
            qty,
            date,
